...
同步 SHALL 由三条互相独立的路径触发，不得只挂在「会话列表被重新读取」上：① 列表刷新之后；② `tabGroups.onDidChangeTabs`（切换/开关标签）；③ 有候选标签存在时的兜底轮询（3 秒间隔、5 分钟封顶）。候选会话不在本地缓存里时，插件 SHALL 自行拉取一次会话列表再判定。
...
#### Scenario: 切换标签就能触发同步（不依赖侧边栏被重新读取）

- **GIVEN** 一个标题为 `Codex` 的已绑定标签，且侧边栏自始至终没有被重新读取（没有 load）
- **WHEN** 触发一次 `onDidChangeTabs`
- **THEN** 插件自行拉取会话列表并完成同步（关闭并用该标签自己的 resource 重开）
